...
| `honeypot-canarytokens` ([docker-compose.canarytokens.yml](docker-compose.canarytokens.yml)) | **home** | self-hosted honeytoken platform (#1426) -- planted-artifact deception, not a listening protocol decoy; `canarytokens-adapter` translates its webhook alerts into this repo's shared JSON event shape |

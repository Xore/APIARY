/* DOMPurify 3.4.13 -- vendored (#1285/#1286), minified, unmodified.
 *
 * Sanitizes marked.js's HTML output before hp-ghidra-markdown.js sets it
 * via innerHTML -- this is AI-generated text describing a real malware
 * sample, ultimately influenced by attacker-controlled strings/behavior
 * the model observed while analyzing it, so it gets the same "never trust
 * this" treatment as every other untrusted field this dashboard renders.
 * A hand-rolled tag/attribute allowlist was considered and rejected: HTML
 * sanitization has a long history of subtle bypasses (mutation XSS,
 * encoding tricks) that a few dozen lines of ad hoc DOM-walking will not
 * reliably catch, so this uses the same widely-audited library the
 * marked.js docs themselves point to for this exact pairing. Exposes a
 * UMD global, window.DOMPurify. See DOMPURIFY-LICENSE.txt for the
 * upstream Apache-2.0 license (dual-licensed, MPL-2.0 also offered
 * upstream).
 */

/* /payload-workbench/{sha256} is one of hp-dynamic-nav.js's DYNAMIC_ROUTES:
   navigating from one payload's workbench page to another's swaps in a
   fresh [data-wb-root] without ever re-fetching this script (already
   loaded from the first visit) -- run() below re-queries the live DOM and
   re-fires both loads on every "hp-dynamic-nav" event, not just once at
   initial script load, so the second payload's "Correlated runs" skeleton
   actually hydrates instead of sitting there until a full reload. Every
   listener here is attached to an element inside [data-wb-root] itself
   (never document), so the old page's listeners are discarded for free
   along with the old DOM when hp-dynamic-nav.js swaps it out -- no
   duplicate-registration risk the way a document-level listener would
   have, so nothing needs hoisting outside run() the way
   hp-payload-analysis.js's retry-button listeners do. */
(() => {
  "use strict";
  let pollTimer = 0;

...
type attackerPage struct {
	...
	// Correlation is populated only for the attacker-correlation-body
	// fragment. attackerData deliberately leaves it empty so Elasticsearch
	// never gates the identifier-aware profile and its in-memory evidence.
	Correlation ipCorrelation
	// Block is populated only for attacker-block-body for the same reason:
	// ipBlockManager.get is a live Elasticsearch round trip. ConfirmedMalicious
	// is local informational context and remains on the immediate page.
	Block              ipBlockRecord

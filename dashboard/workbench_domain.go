...
func workbenchRegistry(classification payloadClassification) []workbenchAnalyzer {
	...
	items := []workbenchAnalyzer{
		...
		// Still Available: false and still absent from submitWorkbenchChild's
		// switch - that part of #78 has not changed. What changed underneath
		// it: ghidra-worker.py now runs Rev-Deck's own automated triage
		// automatically, as one enrichment inside the "ghidra" analyzer above
		// (its result carries a "revdeck" field), gated by REVDECK_API_BASE on
		// the worker host, which this Go process cannot see or health-check.
		// This entry is specifically the *standalone*, independently
		// orchestrated adapter the workbench selection UI would need - its own
		// submission path and result link - and that genuinely does not exist
		// yet, so the old "backend contract is not yet implemented" reason is
		// now stale and has been reworded to say why instead.
		{ID: "revdeck", DisplayName: "Rev\u00b7Deck / GhidrAssist", Description: "Runs automatically as part of the Ghidra analyzer above when REVDECK_API_BASE is configured on the worker (#78); not yet a standalone adapter selectable on its own here.", AcceptedKinds: []string{"executable", "library"}, Availability: "unavailable", Available: false, Applicable: codeLike, Reason: "no standalone submission path or health-check exists for this adapter yet - see the ghidra analyzer's own result for revdeck output (#78)", ResultLinkShape: "", RequiredRole: "admin", Confirmation: "none", Concurrency: "shared-gpu", LocalOnly: true, GPU: true, DefaultOptions: defaultWorkbenchOptions("ghidra"), OptionSchema: optionSchema},
	}
	return items
}

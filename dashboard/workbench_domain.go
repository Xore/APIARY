...
func workbenchRegistry(classification payloadClassification) []workbenchAnalyzer {
	...
	items := []workbenchAnalyzer{
		...
		{ID: "revdeck", DisplayName: "Rev\u00b7Deck / GhidrAssist", Description: "Reserved typed adapter for the verified contract tracked in #78.", AcceptedKinds: []string{"executable", "library"}, Availability: "unavailable", Available: false, Applicable: codeLike, Reason: "backend contract is not yet implemented (#78)", ResultLinkShape: "", RequiredRole: "admin", Confirmation: "none", Concurrency: "shared-gpu", LocalOnly: true, GPU: true, DefaultOptions: defaultWorkbenchOptions("ghidra"), OptionSchema: optionSchema},
	}
	return items
}

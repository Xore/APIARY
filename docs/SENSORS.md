...
| **wordpot** | HTTP 8081 | raw tunnel | vendored `gbrindisi/wordpot` WordPress/CMS decoy (#1421) -- login-page/plugin/theme/timthumb/recent-backups/author-enumeration probe bait, deeper CMS-specific fidelity than beelzebub's own shallow WordPress example config; `wordpot_patch.py` swaps its plain-text logger for JSON and preserves the source port on every request (upstream drops it, same class of gap hellpot's own patch closed) -- no Traefik hostname, real-world WP mass-scanners probe by IP:port not DNS enumeration, and this stack's proven via_port join needs no new header-trust surface -- ES-only from day one |

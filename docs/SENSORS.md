...
| **endlessh** | SSH 19024 (own port, not cowrie's) | raw tunnel + PROXY, public **2022** (#1509 -- not 2222, which is this VPS's own real sshd, confirmed live before picking a port) | SSH pre-auth tarpit, Go port of `skeeto/endlessh`'s core trick — sends random non-"SSH-" banner lines forever, real handshake never begins. Own port deliberately, not cowrie's: diverting an attacker here instead of cowrie's real fake-shell capture would trade depth for a cheap time-waste (#246) |
...
| **hellpot** | HTTP 8080 | raw tunnel + Traefik (`www`/bare-domain/`static`, #1509 -- deliberately routed here, replacing a long-dead `socat-static` bridge; no *dedicated* honeypot hostname, this piggybacks the top-level domain's catch-all instead) | vendored `yunginnanet/HellPot` bot tarpit (#1419) -- streams endless Markov-chain garbage to any request, punishing scanners/scrapers rather than just logging them; upstream's own X-Real-IP trust is unconditional and client-spoofable -- confirmed live -- so `hellpot/router_patch.py` removes it and this stack resolves the real IP via `ip-enrichment-worker`'s via_port join instead (raw-tunnel path) or Traefik's XFF (Traefik-routed path), same as beelzebub/http-honeypot -- ES-only from day one |
| **elasticpot** | HTTP 9201 (own port -- multipot's own hand-rolled Elasticsearch decoy already owns 9200, see docker-compose.elasticpot.yml) | raw tunnel | vendored `gitlab.com/bontchev/elasticpot` (#1423), a second, deliberately distinct Elasticsearch decoy (own persona/asset_id, own container name, own port -- see docker-compose.elasticpot.yml's naming-care note; multipot's pre-existing 9200 decoy is itself self-written, flagged as a separate follow-up decision rather than replaced here); patched (`elasticpot/no_egress_patch.py`) to stop an unconditional startup call to `https://ident.me` that crashed the process outright when outbound internet wasn't reachable -- ES-only from day one |
| **galah** | HTTP 8888 (home), public 8889, plus Traefik `hub.<domain>` (#1511 -- galah's raw port isn't on Cloudflare's proxied non-standard-port allowlist, confirmed live unreachable through the domain otherwise) | raw tunnel + Traefik | vendored `0x4d31/galah` LLM-powered HTTP honeypot (#1420) -- generates a realistic fake response to any request via the shared Ollama instance, reached only through `galah-llm-broker`, a small purpose-built reverse proxy that's the sole bridge onto `honeypot-llm` for this sensor (galah's own container never joins that network, same invariant beelzebub's static-only choice preserved); model `qwen2.5:7b-instruct-q4_K_M`, not any already-pulled Qwen3/3.5 model -- galah's vendored LLM client has no way to send Ollama's `think: false` field, and a live test showed Qwen3.5's thinking mode running for 90s+ without stopping. `timeout_patch.py` raises galah's own hardcoded 10s response deadline to 45s and `galah-llm-broker`'s `UPSTREAM_TIMEOUT_SECONDS` to 40s (#1513) -- a cold-loaded model (Ollama's default 5-minute idle unload, the normal case for a honeypot) measured 15.1s of load_duration alone, comfortably past the original 8-10s ceiling and the actual cause of every real hit returning HTTP 500 instead of a decoy response. `xff_trust_patch.py` (#1511) trusts `X-Forwarded-For` from the WireGuard tunnel peer only, needed now that galah is reachable both raw-port (via_port resolution, unchanged) and Traefik-routed (XFF resolution, new) -- ES-only from day one |
| **sentrypeer** | SIP 5070 (public), 5060 internally -- a port shift, not a collision: dionaea already binds host 5060 for its own generic SIP banner-grab, sentrypeer's own listener is hardcoded to 5060 internally with no config option | raw tunnel | vendored `SentryPeer/SentryPeer` SIP/VoIP fraud-detection honeypot (#1424) -- real SIP protocol parsing (osip2), not a banner-grab; `sentrypeer/nul_byte_crash_patch.py` fixes a real, confirmed-live crash where any packet with an embedded NUL byte panicked the SIP listener thread; peer-to-peer bad-actor sharing and its RESTful API both left disabled (same no-third-party-sharing call as `docs/community-threat-intel-sharing.md`); no Traefik hostname (SIP has no HTTP-style header a proxy could set) -- ES-only from day one |
| **wordpot** | HTTP 8081 | raw tunnel, public **8082** (#1509), plus Traefik `news.<domain>` (#1512 -- wordpot's raw port isn't on Cloudflare's proxied non-standard-port allowlist either, same finding as galah/#1511) | vendored `gbrindisi/wordpot` WordPress/CMS decoy (#1421) -- login-page/plugin/theme/timthumb/recent-backups/author-enumeration probe bait, deeper CMS-specific fidelity than beelzebub's own shallow WordPress example config; `wordpot_patch.py` swaps its plain-text logger for JSON and preserves the source port on every request (upstream drops it, same class of gap hellpot's own patch closed), and now also trusts `X-Forwarded-For` from the WireGuard tunnel peer only (#1512) -- via_port join still resolves the raw-port path unchanged, XFF resolves the new Traefik-routed path, the same dual-path shape galah/#1511 uses -- ES-only from day one |
| **mailoney** | SMTP 25 | raw tunnel | vendored `awhitehatter/mailoney` (#1422), takes over port 25 from multipot's own retired self-written SMTP handler -- real DATA/message-body capture (multipot's handler never had this), EHLO/AUTH PLAIN credential capture. Upstream persists sessions to a SQLAlchemy DB only, no JSON log of its own; `mailoney/json_log_patch.py` adds a flat per-event JSON-line sink matching this stack's convention, and fixes a real found-live bug along the way -- upstream lowercases the whole request line (including the AUTH PLAIN base64 payload) before decoding it, silently corrupting any captured credential with an uppercase letter -- ES-only from day one |
| **canarytokens** | management UI/API 19426 (WireGuard-tunnel only); HTTP-channel 19427 also public via VPS Traefik `Host(\`cdn.honeypot.example\`)` -> `socat-hp-canarytokens` (#1487) | internal, `canarytokens-adapter`'s own webhook | self-hosted `thinkst/canarytokens` (#1426), a different deception *category* -- planted honeytokens (documents, URLs, DNS names, credentials) that alert the instant something touches them, not a listening decoy protocol. Token creation IS scriptable despite the #1415 research pass's premise: a real `POST` endpoint exists at a deliberately non-guessable path (`frontend/app.py`'s `ROOT_API_ENDPOINT`, an anti-scraping measure -- not `/api/generate`, confirmed live by actually calling it, not by reading the OpenAPI schema alone). #1487 uses it: the dashboard's Settings > Canarytokens pane creates PDF/Word/Excel/custom-image/Windows-Folder/QR tokens on demand for external use (planted anywhere outside this honeypot, not just its own personas), and the operator downloads the resulting artifact. The public `cdn` hostname is what those artifacts' trigger URLs actually resolve to; it's deliberately not named `canary`/`token*` so an inspected artifact doesn't out itself. `#1427`'s cross-honeypot breadcrumbs (baked into persona files at deploy time) remain a separate, still-internal-only use of the same platform. |
| **suricata** | (sniffs all traffic, runs on the **VPS**) | — | IDS over every honeypot packet → eve.json → ELK, pcap → Arkime |

multipot cedes FTP/MySQL/MSSQL/Mongo to Dionaea, and SMTP to mailoney, automatically

...
| **canarytokens** | management UI/API 19426, HTTP-channel 19427 (WireGuard-tunnel only, no public exposure yet) | internal, `canarytokens-adapter`'s own webhook | self-hosted `thinkst/canarytokens` (#1426), a different deception *category* -- planted honeytokens (documents, URLs, DNS names, credentials) that alert the instant something touches them, not a listening decoy protocol. Token creation IS scriptable despite the #1415 research pass's premise: a real `POST` endpoint exists at a deliberately non-guessable path (`frontend/app.py`'s `ROOT_API_ENDPOINT`, an anti-scraping measure -- not `/api/generate`, confirmed live by actually calling it, not by reading the OpenAPI schema alone), central to #1427's plan to plant tokens at scale. No tokens planted and no VPS/portbridge wiring yet -- that's #1427's job; this issue's scope is standing the platform up and getting fired-token alerts into Elasticsearch via `canarytokens-adapter` (this repo's own small webhook receiver, since canarytokens itself never writes JSON Filebeat can tail) |

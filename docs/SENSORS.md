...
| **canarytokens** | management UI/API 19426 (WireGuard-tunnel only); HTTP-channel 19427 also public via VPS Traefik `HostRegexp(\`^[a-z0-9]+\.honeypot\.example$\`)` (a wildcard on the bare apex, not a fixed Host()) -> `socat-hp-canarytokens` (#1487) | internal, `canarytokens-adapter`'s own webhook | self-hosted `thinkst/canarytokens` (#1426), a different deception *category* -- planted honeytokens (documents, URLs, DNS names, credentials) that alert the instant something touches them, not a listening decoy protocol. Token creation IS scriptable despite the #1415 research pass's premise: a real `POST` endpoint exists at a deliberately non-guessable path (`frontend/app.py`'s `ROOT_API_ENDPOINT`, an anti-scraping measure -- not `/api/generate`, confirmed live by actually calling it, not by reading the OpenAPI schema alone). #1487 uses it: the dashboard's Settings > Canarytokens pane creates PDF/Word/Excel/custom-image/Windows-Folder/QR tokens on demand for external use (planted anywhere outside this honeypot, not just its own personas), and the operator downloads the resulting artifact. `canarydrop.generate_random_hostname()` prepends each token's own random value as a subdomain of `CANARY_PUBLIC_HOSTNAME` -- the wildcard match (not a dedicated `cdn`/`canary`/`token*` subdomain) is what a wildcard-cert-covered domain and an actual, previously-unknown-at-DNS-time random label both require; see `docs/CGNAT-DEPLOYMENT.md`'s DNS section. `#1427`'s cross-honeypot breadcrumbs (baked into persona files at deploy time) remain a separate, still-internal-only use of the same platform. |

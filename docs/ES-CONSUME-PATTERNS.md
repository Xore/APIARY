...
| correlator-worker | pending (pattern 2 as-is; candidate only if it ever grows persisted position) |
| payload-inventory-worker | pending (pattern 2 variant; nothing to migrate unless scan becomes incremental) |
| agent-intrusion worker | pending (pattern 2 exemplar; could vendor an upsert helper if duplication ever appears elsewhere) |
| es-results-importer | pending (pattern 2 variant; mtime discipline documented) |

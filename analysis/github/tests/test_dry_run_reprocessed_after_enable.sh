...
# (GH_PAT, GITHUB_REPO, real dirs), including across the disable/enable
# flip this test drives. The old refuse-guard closed that leak by refusing
# to run on any host carrying the file, which made this test permanently
# red on the very runner it is routed to (#2461). Pointing the variable at
# an empty fixture inside $work closes the same leak deterministically
# instead: nothing host-owned can ever be sourced.
: >"$work/host-env"

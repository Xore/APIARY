...
| `rex86_run_deepseek_v2_full.sh` | `rex86_run_base_model.sh`'s pipeline for one specific model, `deepseek-ai/DeepSeek-Coder-V2-Instruct` (the full 235.74B model) -- needs its own script because the ~940GB peak disk usage (raw snapshot + f16 GGUF coexisting mid-conversion) is large enough that it runs in a separate throwaway container against its own `/var/rex86-large-models` dir instead of sharing the main container's work dir. |

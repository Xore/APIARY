...
run_direct() {
  ...
  # pipeline has actually succeeded -- the old "rm -f $result" up front
  # destroyed the last-known-good result even when the launch below failed
  # (#2055), and an unconditional "tee $result" left a truncated file in
  # place on a mid-eval failure that a later run's exists-check trusted as
  # done.

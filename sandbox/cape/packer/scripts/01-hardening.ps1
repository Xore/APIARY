...
# 01-hardening.ps1 — Packer provisioner, step 1 of win11-cape.pkr.hcl's build.
#
# Copied verbatim from sandbox/windows/packer/scripts/01-hardening.ps1 (#315)
# rather than referenced across trees: every setting here (Defender off,
# Windows Update off, telemetry off, UAC off, firewall off, Chocolatey) is
# CAPE-recommended guest prep too (docs/book/src/installation/guest/
# network.rst on the real upstream checkout says the same about firewall/
# updates), not sandbox/windows-specific. If this drifts from that copy,
# prefer pulling the fix here rather than re-diverging further.
#

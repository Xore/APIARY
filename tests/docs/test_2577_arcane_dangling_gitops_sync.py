...
store, leaving one more live `gitops_syncs` row than manifest entries.
...
pure diff/plan logic against a fixture standing in for the real store:
the current manifest, plus one synthetic `honeypot-wordpot` row shaped
like the orphan the issue describes. Counts are derived from the
manifest at runtime -- the manifest is free to grow or shrink.
"""
...
def _live_syncs_with_orphan(manifest_entries):
    """Fixture store: one live sync per manifest entry, plus the dangling
...
def test_manifest_has_no_wordpot_entry(manifest_entries):
    ...
    assert len(names) == len(manifest_entries), "duplicate syncName in manifest"
...
def test_live_store_matches_manifest_after_removing_orphan(mod, manifest_entries):
    ...
    # migration, live rows == manifest entries, and the remaining names
    # match exactly.
    manifest_names = mod.manifest_sync_names(MANIFEST_PATH)
    live = _live_syncs_with_orphan(manifest_entries)
    ...
    assert len(live) == len(manifest_entries) + 1

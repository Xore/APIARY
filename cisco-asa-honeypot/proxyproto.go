...
// wrapped http.Server sees the real client address.
...
// address is kept. A short read deadline guards against a client that connects
// but never sends the promised header.
func decodeProxy(c net.Conn, enabled bool) net.Conn {
	...
	remote := parseProxy(r)
	c.SetReadDeadline(time.Time{}) // clear; the http.Server sets its own timeouts

...
// real attacker address — so r.RemoteAddr (and clientIP) see the true IP
// before net/http ever parses the request. (#2099: the decode itself must
// never run on the shared accept-loop goroutine.)
//
...
// single, unbuffered accept loop and only spawns a per-connection
// goroutine after Accept() returns; decoding the PROXY header synchronously
// inside Accept() (the previous behavior) blocked that shared loop for up
// to the decode's own 5s deadline on every connection, including a
// slow/silent one -- a trivial, near-zero-bandwidth DoS against the whole
// listener (#1346 fixed this in cisco-asa; #2099 ports it here).

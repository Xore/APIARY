 ...
 * is open, so this loads once per mount.
 *
 * #1564: /attackers is now a shell-wide SPA route (hp-dynamic-nav.js), so
 * this file's own top-level IIFE run only ever fires once, the first time
 * ANY page needing it is visited (hp-dynamic-nav.js's mergeExtraContent
 * dedups <script src> by src). initAttackerGraph is exposed as
 * window.initHoneypotAttackerGraph -- same convention hp-app.js's own
 * window.initHoneypotMaps/window.initHoneypotCharts already use -- so
 * hp-app.js's mountPage can re-run it against the freshly-mounted DOM on
 * every later navigation to /attackers. Calling it on a mount that carries
 * no [data-attacker-graph-url] (i.e. every other page) is a cheap no-op via
 * the same early return the original one-shot version already had. Old
 * cytoscape instances/ResizeObservers on a page navigated away from are
 * left to be garbage-collected along with their now-detached canvas -- the
 * same accepted, documented cost hp-kill-chain.js's own initHoneypotCharts
 * already takes for ECharts instances on repeat navigation.
 */
(() => {
  "use strict";

  function initAttackerGraph() {
  ...
  initAttackerGraph();
  window.initHoneypotAttackerGraph = initAttackerGraph;
})();

...
async function openViewer(page: Page, palette: string, mode: string) {
  ...
  await expect(page.locator('.novnc-shadcn')).toBeVisible()
  await expect(page.locator('.novnc-shadcn canvas')).toBeAttached({ timeout: 20_000 })
  await page.evaluate(
    ({ palette, mode }) => {
      localStorage.setItem('hp-theme', mode)
...
async function chromeColors(page: Page) {
  return page.locator('.novnc-shadcn').evaluate((host) => {
    const toolbar = host.querySelector('.hp-vnc-status') as HTMLElement
    const frame = host.querySelector('.hp-vnc-canvas-wrap') as HTMLElement
    ...
    const actual = {
      ...
      toolbarForeground: getComputedStyle(toolbar).color,
      toolbarBorder: getComputedStyle(toolbar).borderTopColor,
    ...
    const expected = {
      toolbarBackground: token('--card'),
      toolbarForeground: token('--card-foreground'),
      toolbarBorder: token('--border'),
...
for (const entry of cases) {
  test(`${entry.palette}/${entry.mode}/${entry.width} themes noVNC chrome and starts RFB`, async ({ page }) => {
    ...
    await expect(page.locator('.novnc-shadcn canvas')).toBeAttached()

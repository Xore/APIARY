...
it('shows the loading state then backend-shaped CIDR correlation with shadcn cards, table and pivots', async () => {
  ...
  try {
    ...
    const sensorsHeading = Array.from(view.host.querySelectorAll('h2')).find(node => node.textContent === 'Sensors')
    expect(sensorsHeading?.classList.contains('font-semibold')).toBe(true)
    expect(sensorsHeading?.classList.contains('leading-none')).toBe(true)
    expect(sensorsHeading?.classList.contains('tracking-tight')).toBe(true)
    expect(sensorsHeading?.parentElement?.classList.contains('p-6')).toBe(true)
    expect(sensorsHeading?.parentElement?.parentElement?.classList.contains('rounded-xl')).toBe(true)
    expect(view.host.textContent).toContain('12 correlated events for this network')
    expect(view.host.textContent).toContain('generated')

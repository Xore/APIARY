...
func TestGETLogonHTMLQueryStringIsCapturedInLoggedEvent(t *testing.T) {
	path := filepath.Join(t.TempDir(), "out.json")
	h := &webvpnHandler{log: newLogger(path), port: 8443}

	req := httptest.NewRequest("GET", "/+CSCOE+/logon.html?reason=1&Login=1", nil)
	w := httptest.NewRecorder()
	h.ServeHTTP(w, req)
	...
	if e.Query != "reason=1&Login=1" {
		t.Fatalf("Query = %q, want %q", e.Query, "reason=1&Login=1")
	}
}

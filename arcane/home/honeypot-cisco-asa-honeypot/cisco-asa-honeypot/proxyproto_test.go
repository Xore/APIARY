...
// TestProxyListenerAcceptDoesNotBlockOnSlowPeer covers #1346: Accept() must
// return as soon as the underlying listener accepts a connection, even if
// the peer then sends nothing at all -- the PROXY header decode (and its
...
// correctness half of #1346's fix: net/http captures RemoteAddr() before
// the TLS handshake (and therefore before any Read) even starts, so a fix
// that only decoded the PROXY header lazily on first Read would silently
// break attacker-IP attribution -- RemoteAddr() must itself trigger (and
// wait for) the decode.
func TestProxyConnRemoteAddrResolvesRealAddressBeforeAnyRead(t *testing.T) {
	...
	// RemoteAddr(), not Read, is the first call -- exactly net/http's own
	// ordering (c.remoteAddr = c.rwc.RemoteAddr().String() runs before the
	// TLS handshake).
	addr := pc.RemoteAddr()
